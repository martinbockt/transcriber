...
export const AudioPlayer = forwardRef<AudioPlayerRef, AudioPlayerProps>(function AudioPlayer(
  ...
  const { isPlaying, currentTime, duration, play, pause, seek, skipForward, skipBackward, error } =
    useAudioPlayer();
  ...
  return (
    <div className={className}>
      <div className="bg-muted/30 border-border rounded-lg border p-3 backdrop-blur-xs">
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="sm"
            onClick={handleSkipBack}
            className="hover:bg-primary/10 h-9 w-9 rounded-full p-0 transition-colors"
            disabled={!duration}
          >
